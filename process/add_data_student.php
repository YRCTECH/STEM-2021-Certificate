<?php
    ...
    if(isset($_POST['insert'])){
        ...
        $sql = "INSERT INTO `student_certificate` (`c_id`, `c_ref_code`, `c_type`, `c_grade`, `c_team`, `c_school`,`c_name`, `c_awards`, `c_awards_detail`) 
        ...
        if($query){
            ...
            if($query2){
                header('location:../admin_print_student.php?status=success');
            }else{
                header('location:../admin_print_student.php?status=error');
            }
        }else{
            header('location:../admin_print_student.php?status=error');
        }
